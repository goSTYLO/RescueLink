...
- [x] Migration applied (`auto_assignment_*` columns present)
- [x] Backend + AI + Web + Mobile all running
- [x] Seed loaded (or you know your team roster)
- [x] `AI_LOW_CONFIDENCE_THRESHOLD=0.7` in `Backend/.env`
- [x] At least one `drrmo` team `available`, one `pnp` team `available`
...
| Step | Action                              | Expected                                                   |
| ---- | ----------------------------------- | ---------------------------------------------------------- |
| 1    | Open assigned incident              | Stepper starts at **En Route**; primary action is **Mark On Scene** |
| 2    | Tap **Mark On Scene**               | `PATCH /api/dispatches/me/status` 200                      |
| 3    | DB                                  | **Your** `dispatches.response_status` updated (title case) |
| 4    | Timeline + WS                       | Coordination note + `responder:status_changed` broadcast   |
